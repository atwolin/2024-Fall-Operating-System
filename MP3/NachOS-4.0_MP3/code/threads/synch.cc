...
void Semaphore::P() {
    ...
    DEBUG(dbgThread, "thread " << currentThread->getName()
                               << " In Semaphore::P(), "
...
void Semaphore::V() {
    ...
    // re-enable interrupts
    (void)interrupt->SetLevel(oldLevel);
    // kernel->scheduler->CheckPreempt_btMultipleReadyQueue();
    // kernel->scheduler->CheckPreempt_inSingleReadyQueue();
}
