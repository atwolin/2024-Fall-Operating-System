...
void Thread::Yield() {  // time sharing
    Thread *nextThread;
    ...
    TimeUpdate_RunningToReady(kernel->stats->totalTicks);
...
//----------------------------------------------------------------------
void Thread::Sleep(bool finishing) {  // finish v I/O etc.
    Thread *nextThread;
    ...
    DEBUG(dbgThread,
          "Sleeping thread: " << name << ", finishing? " << finishing);
    ...
    DEBUG(dbgZ, "[E] Tick ["
                    ...
                    << nextThread->getID()
                    << "] is now selected for execution, thread ["
                    << this->getID() << "] is replaced, and it has executed ["
                    << kernel->stats->totalTicks - timeStamp_startRunning
                    << "] ticks");
    if (!finishing) {
        TimeUpdate_RunningToWaiting(kernel->stats->totalTicks);
        DEBUG(dbgZ, "[D] Tick ["
                        << kernel->stats->totalTicks << "]: Thread ["
                        << this->getID()
                        << "] update approximate burst time, from: ["
                        << last_approximatedBurstTime << "], add ["
                        << kernel->stats->totalTicks - timeStamp_startRunning
                        << "], to [" << curr_approximatedBurstTime << "]");
    }
...
void Thread::StackAllocate(VoidFunctionPtr func, void *arg) {
    stack = (int *)AllocBoundedArray(StackSize *
                                     sizeof(int));  // Why "* sizeof(int)"?
