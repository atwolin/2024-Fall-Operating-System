...
int OpenFile::ReadAt(char *into, int numBytes, int position) {
    ...
    for (i = firstSector; i <= lastSector; i++) {
        // DEBUG(dbgFile,"    In OpenFile::ReadAt(), into FileHeader::ByteToSector()");
        kernel->synchDisk->ReadSector(hdr->ByteToSector(i * SectorSize),
                                      &buf[(i - firstSector) * SectorSize]);
        // DEBUG(dbgFile,"    In OpenFile::ReadAt(), return from FileHeader::ByteToSector()");
    }

    // copy the part we want
    bcopy(&buf[position - (firstSector * SectorSize)], into, numBytes);
    delete[] buf;
    // DEBUG(dbgFile,"End OpenFile::ReadAt()");
    return numBytes;
}
...
int OpenFile::WriteAt(char *from, int numBytes, int position) {
    ...
    memset(buf, 0,
           sizeof(char) * numSectors *
               SectorSize);  // dummy operation to keep valgrind happy
