        ...
        case SyscallException:
            switch (type) {
                ...
                case SC_Open:
                    ...
                    */
                    DEBUG(dbgFile, "In ExceptionHandler:case SC_Open.");
                    val = kernel->machine->ReadRegister(
                        4);  // Retrieve file name address.
                    {
                        char *filename =
                            &(kernel->machine
                                  ->mainMemory[val]);  // Retrive file name.
                        DEBUG(
                            dbgFile,
                            "In ExceptionHandler:case SC_Open, into SysOpen.");
                        fileID = SysOpen(
                            filename);  // Success: get file ID / Fail: get -1
                        DEBUG(dbgFile,
                              "In ExceptionHandler:case SC_Open, return from "
                              "SysOpen.");
                        kernel->machine->WriteRegister(
                            2, fileID);  // Write file ID into register.
                    }
                    kernel->machine->WriteRegister(
                        PrevPCReg, kernel->machine->ReadRegister(PCReg));
                    kernel->machine->WriteRegister(
                ...
                case SC_Read:
                    ...
                    */
                    DEBUG(dbgFile, "In ExceptionHandler:case SC_Read.");
                    val = kernel->machine->ReadRegister(4);
                    {
                        ...
                        DEBUG(
                            dbgFile,
                            "In ExceptionHandler:case SC_Read, into SysRaed()");
                ...
                case SC_Write:
                    /*
                    #define SC_Write 8
                    Write “size” characters from the buffer into the file, and
                    return the number of characters actually written to the file
                    */
                    DEBUG(dbgFile, "In ExceptionHandler:case SC_Write.");
                    val = kernel->machine->ReadRegister(4);
                    {
                        char *buffer = &(kernel->machine->mainMemory[val]);
                        numChar = kernel->machine->ReadRegister(5);
                        ...
                        numChar = SysWrite(
                            buffer, numChar,
                            fileID);  // Success: get number of character
                                      // written into the file / Fail: get -1
                        DEBUG(dbgFile,
                              "In ExceptionHandler:case SC_Write, return from "
                              "SysWrite()");
                        kernel->machine->WriteRegister(
                            2, numChar);  // Write result into register.
                    }
                    kernel->machine->WriteRegister(
                        PrevPCReg, kernel->machine->ReadRegister(PCReg));
                    kernel->machine->WriteRegister(
                ...
                case SC_Close:
                    ...
                    Close the file.
                    */
                    DEBUG(dbgFile, "In ExceptionHandler:case SC_Close.");
                    val = kernel->machine->ReadRegister(4);
                    DEBUG(dbgFile,
